...
function draw() {
  ...
  if(repeat){
    ...
    {
      vertex(i*zoom, map(spectrum[i], -130, -30, height, height/2) );
      //undo comment to see peaks detected by a simpler peakfinding algorithm
      /*
      if(spectrum[i]>spectrum[i-1] && spectrum[i]>spectrum[i+1]){
        fill('blue');
        ellipse(i*zoom, map(spectrum[i], 0, 255, height, height/2),5,5)
        noFill();
      }
      */
    }
    ...
    ellipse(i*zoom, map(spectrum[i+1]-spectrum[i-1], -130, 130, height/2, 0), 5,5);
    vertex(i*zoom, map(spectrum[i+1]-spectrum[i-1], -130, 130, height/2, 0) );
    ...
    if(point1[1]>=0 && point2[1]<0){
      ...
      if(secondDerivative < -1*cutoff){
        ...
        if(peakEnergy>-100){
          fill('red');
          ellipse(peakFrequency*zoom, map(peakEnergy, -130, -30, height, height/2),7,7 );
          noFill();
